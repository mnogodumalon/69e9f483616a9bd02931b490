...
const REPORTINGANFRAGE_FIELDS = [
  { key: 'titel', label: 'Titel der Anfrage', type: 'string/text' },
  { key: 'berichtstyp', label: 'Berichtstyp', type: 'lookup/select', options: [{ key: 'monatsbericht', label: 'Monatsbericht' }, { key: 'quartalsbericht', label: 'Quartalsbericht' }, { key: 'jahresbericht', label: 'Jahresbericht' }, { key: 'ad_hoc_bericht', label: 'Ad-hoc-Bericht' }, { key: 'kpi_bericht', label: 'KPI-Bericht' }, { key: 'sonstiges', label: 'Sonstiges' }] },
  { key: 'bereich', label: 'Bereich / Abteilung', type: 'lookup/select', options: [{ key: 'vertrieb', label: 'Vertrieb' }, { key: 'marketing', label: 'Marketing' }, { key: 'finanzen', label: 'Finanzen' }, { key: 'personal', label: 'Personal' }, { key: 'it', label: 'IT' }, { key: 'produktion', label: 'Produktion' }, { key: 'einkauf', label: 'Einkauf' }, { key: 'sonstiges_bereich', label: 'Sonstiges' }] },
  { key: 'zeitraum_von', label: 'Zeitraum von', type: 'date/date' },
  { key: 'zeitraum_bis', label: 'Zeitraum bis', type: 'date/date' },
  { key: 'anforderer_vorname', label: 'Vorname des Anforderers', type: 'string/text' },
  { key: 'anforderer_nachname', label: 'Nachname des Anforderers', type: 'string/text' },
  { key: 'anforderer_email', label: 'E-Mail des Anforderers', type: 'string/email' },
  ...
  { key: 'status', label: 'Status', type: 'lookup/select', options: [{ key: 'offen', label: 'Offen' }, { key: 'in_bearbeitung', label: 'In Bearbeitung' }, { key: 'abgeschlossen', label: 'Abgeschlossen' }, { key: 'abgelehnt', label: 'Abgelehnt' }] },
  { key: 'faelligkeitsdatum', label: 'Fälligkeitsdatum', type: 'date/date' },
  { key: 'kennzahlen', label: 'Relevante Kennzahlen', type: 'multiplelookup/checkbox', options: [{ key: 'umsatz', label: 'Umsatz' }, { key: 'kosten', label: 'Kosten' }, { key: 'gewinn', label: 'Gewinn' }, { key: 'kundenzahl', label: 'Kundenzahl' }, { key: 'mitarbeiterzahl', label: 'Mitarbeiterzahl' }, { key: 'conversion_rate', label: 'Conversion Rate' }, { key: 'nps', label: 'NPS' }, { key: 'sonstiges_kz', label: 'Sonstiges' }] },
  { key: 'beschreibung', label: 'Beschreibung / Anforderungen', type: 'string/textarea' },
  { key: 'anhang', label: 'Anhang', type: 'file' },
];

const ENTITY_TABS = [
  { key: 'reporting_anfrage', label: 'Reporting-Anfrage', pascal: 'ReportingAnfrage' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('reporting_anfrage');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'reporting_anfrage': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'reporting_anfrage': return (data as any).reportingAnfrage as ReportingAnfrage[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'reporting_anfrage': return REPORTINGANFRAGE_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'reporting_anfrage': return {
        create: (fields: any) => LivingAppsService.createReportingAnfrageEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateReportingAnfrageEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteReportingAnfrageEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'reporting_anfrage' || dialogState?.entity === 'reporting_anfrage') && (
        <ReportingAnfrageDialog
          open={createEntity === 'reporting_anfrage' || dialogState?.entity === 'reporting_anfrage'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'reporting_anfrage' ? handleUpdate : (fields: any) => handleCreate('reporting_anfrage', fields)}
          defaultValues={dialogState?.entity === 'reporting_anfrage' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['ReportingAnfrage']}
          enablePhotoLocation={AI_PHOTO_LOCATION['ReportingAnfrage']}
        />
      )}
      {viewState?.entity === 'reporting_anfrage' && (
        <ReportingAnfrageViewDialog
          open={viewState?.entity === 'reporting_anfrage'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'reporting_anfrage', record: r }); }}
        />
      )}
