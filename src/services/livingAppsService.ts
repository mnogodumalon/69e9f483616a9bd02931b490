...
import type { ReportingAnfrage, CreateReportingAnfrage } from '@/types/app';
...
export class LivingAppsService {
  // --- REPORTING_ANFRAGE ---
  static async getReportingAnfrage(): Promise<ReportingAnfrage[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.REPORTING_ANFRAGE}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as ReportingAnfrage[];
    return enrichLookupFields(records, 'reporting_anfrage');
  }
  static async getReportingAnfrageEntry(id: string): Promise<ReportingAnfrage | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.REPORTING_ANFRAGE}/records/${id}`);
    const record = { record_id: data.id, ...data } as ReportingAnfrage;
    return enrichLookupFields([record], 'reporting_anfrage')[0];
  }
  static async createReportingAnfrageEntry(fields: CreateReportingAnfrage) {
    return callApi('POST', `/apps/${APP_IDS.REPORTING_ANFRAGE}/records`, { fields: cleanFieldsForApi(fields as any, 'reporting_anfrage') });
  }
  static async updateReportingAnfrageEntry(id: string, fields: Partial<CreateReportingAnfrage>) {
    return callApi('PATCH', `/apps/${APP_IDS.REPORTING_ANFRAGE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'reporting_anfrage') });
  }
  static async deleteReportingAnfrageEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.REPORTING_ANFRAGE}/records/${id}`);
  }
